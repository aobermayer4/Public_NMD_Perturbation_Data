# NMD Metadata Explorer: R 4.0.5 / Shiny 1.6 Compatibility Build

A local R Shiny application designed to run without `bslib` or Sass, including on older Shiny Server environments such as R 4.0.5 with Shiny 1.6.0. for interactively searching the parsed GEO and ArrayExpress metadata tables produced by:
...
This build deliberately does not list, load, or call `bslib`. It uses the Bootstrap 3 assets bundled with Shiny 1.6.0 and ordinary CSS from `www/app.css`.

This avoids both of the server-side failures commonly seen with the reported environment:

- `bs_theme(version = 5)` requiring Shiny 1.7.0 or newer.
- Sass compilation and cache-directory writes under a read-only Shiny Server application directory.

The installed `bslib` and `sass` packages may remain on the server; this application simply does not use them.

The application only writes exports beneath R's session temporary directory. It does not need write permission in the deployed application directory. The Shiny Server service account does, however, need read permission for the application code and bundled TSV files.

### Recommended preflight check

From the deployed project directory, run:

```r
source("server_preflight.R")
```
